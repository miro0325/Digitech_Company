...
public class BreakableWall : NetworkObject,IDamagable
{
    [SerializeField] private float maxHp;

    private Stats testBaseStat = new();
    private Stats maxStats = new();
    private Stats curStats = new();
    private Stats.Modifier modifier = new();
    ...
    public bool IsInvulnerable => isBroken;

    void Start()
    {
        modifier.OnValueChange += key => modifier.Calculate(key, maxStats, testBaseStat);
        maxStats.OnStatChanged += (key, old, cur) =>
        {
            var diff = cur - old;
            curStats.SetStat(key, x => x + diff);
        };
        testBaseStat.SetStat(Stats.Key.Hp, x => x = maxHp);
    ...
    {
        if (isBroken) return;
        StartCoroutine(DestroyingMesh());
    }

    private IEnumerator DestroyingMesh()
    {
        WaitForSeconds wait = new WaitForSeconds(0.03f);
        WaitForSeconds wait2 = new WaitForSeconds(0.03f);
    ...
    public void Damage(float damage, UnitBase attacker)
    {
        ...
        if(attacker != null && attacker is InGamePlayer)
        {
            var player = attacker as InGamePlayer;
            if (player.Inventory.GetCurrentSlotItem().Key != "Drill") return;
        }
        if (curStats.GetStat(Stats.Key.Hp) <= 0) return;
        photonView.RPC(nameof(SendDamageToOwnerRpc), photonView.Owner, damage *2);
    }
    ...
    protected void SendDamageToOwnerRpc(float damage)
    {
        ...
        Debug.Log(curStats.GetStat(Stats.Key.Hp));
    ...
    public class PartMesh
    {
        ...
        public void MakeGameobject(BreakableWall original)
        {
            ...
            var brokenMesh = GameObject.AddComponent<BrokenMesh>();
            GameObject.layer = LayerMask.NameToLayer("BrokenWall");
            brokenMesh.DestroySmallMesh();
        }
